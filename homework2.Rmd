...
biketable_nonfilter<- bike %>% 
  group_by(year,month) %>% 
  mutate(actual_bikesrecent = mean(bikes_hired))
biketable_nonfilter
  
jan_exp <- biketable[1,6]
feb_exp <- biketable[32,6]
mar_exp <- biketable[61,6]
apr_exp <- biketable[92,6]
may_exp <- biketable[122,6]
jun_exp <- biketable[153,6]
jul_exp <- biketable[183,6]
aug_exp <- biketable[214,6]
sep_exp <- biketable[245,6]
oct_exp <- biketable[275,6]
nov_exp <- biketable[306,6]
dec_exp <- biketable[336,6]

biketable_nonfilter <- biketable_nonfilter %>% 
  mutate(expected_rentals = case_when(month == "jan" ~jan_exp,
                                      month == "feb" ~feb_exp,
                                      month == "mar" ~mar_exp,
                                      month == "apr" ~apr_exp,
                                      month == "may" ~may_exp,
...
biketable <- biketable %>% 
  group_by(year, month) %>% 
  mutate(actual_bikes = mean(bikes_hired))
        
biketable <- biketable %>% 
  mutate(residual_bikes = actual_bikes - expected_rentals)




ggplot(biketable, aes(x = month, y = expected_rentals, group = 1))+
  geom_ribbon()+
  facet_wrap(~year)+
  NULL
...
We should use the median number of rentals as our expected rentals, since the mean could be skewed by extreme values. 
